...
az containerapp create \
  ...
  --registry-username $(az acr credential show --name mcptutorialacr --query "username" -o tsv) \
  --registry-password $(az acr credential show --name mcptutorialacr --query "passwords[0].value" -o tsv) \
  --target-port 8000 \
  --ingress internal \
  --min-replicas 1 \
  --max-replicas 3 \
...
```bash
SERVER_URL=$(az containerapp show --name mcp-weather-server --resource-group mcp-tutorial-rg --query "properties.configuration.ingress.fqdn" -o tsv)
```

Then deploy the web client:
```bash
az containerapp create \
  --name mcp-web-client \
  --resource-group mcp-tutorial-rg \
...
### Get Web Client URL
```bash
WEB_CLIENT_URL=$(az containerapp show --name mcp-web-client --resource-group mcp-tutorial-rg --query "properties.configuration.ingress.fqdn" -o tsv)
echo "Web Client URL: https://$WEB_CLIENT_URL"
```
...
az containerapp revision restart \
  --name mcp-web-client \
  --resource-group mcp-tutorial-rg \
  --revision $(az containerapp revision list --name mcp-web-client --resource-group mcp-tutorial-rg --query "[0].name" -o tsv)
```

...
class ToolRegistry:
    ...
    async def _discover_server_tools(
        ...
        try:
            ...
            # For MCP protocol, we need to send a proper MCP request
            # This is a simplified version - in production you'd use the MCP client
            mcp_request = {
                "jsonrpc": "2.0",
                "id": 1,
                "method": "tools/list",
            ...
            response = await self.http_client.post(
                url, json=mcp_request, headers={"Content-Type": "application/json"}
            )
            ...
            if response.status_code == 200:
                ...
                if "result" in data and "tools" in data["result"]:
                    await self._process_discovered_tools(
                        server_name, config, data["result"]["tools"]
                    )
                else:
                    logger.warning(f"Invalid MCP response from {server_name}: {data}")
            else:
                logger.warning(
